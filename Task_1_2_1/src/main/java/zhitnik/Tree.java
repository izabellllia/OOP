...
class Tree<T> {
    ...
    /**метод, возвращающий высоту дерева. Высота определяется как максимальное количество уровней
     * в дереве. Возвращает результат вызова приватного метода getHeight(root).*/
...
/**Представляет узел в дереве. Каждый узел имеет данные типа T и список дочерних узлов. Он предоставляет
 * методы для получения и установки данных и дочерних узлов, а также для добавления дочернего узла.*/
 class Node<T> {
     ...
     * Создает новый объект Node<T> с переданными данными и
